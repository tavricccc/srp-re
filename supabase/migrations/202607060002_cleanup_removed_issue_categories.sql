-- Clean up database and Cloudinary remnants when issue categories are removed from config.
-- Notion pages are kept as historical backups and marked as deleted through the existing outbox worker.

drop function if exists app_api.run_maintenance_cleanup();
drop function if exists app_private.run_maintenance_cleanup();
...
create or replace function app_api.run_maintenance_cleanup(valid_issue_categories text[] default null)
returns jsonb
language sql
security definer
set search_path = app_private, app_api, public
as $$
  select app_private.run_maintenance_cleanup(valid_issue_categories);
$$;

revoke all on function app_api.run_maintenance_cleanup(text[]) from public, anon, authenticated;
grant execute on function app_api.run_maintenance_cleanup(text[]) to service_role;
