-- Drive core-content and operational-data retention from the config supplied by maintenanceCleanup.

drop function if exists app_api.run_maintenance_cleanup(text[], jsonb);
drop function if exists app_private.run_maintenance_cleanup(text[], jsonb);

create function app_private.run_maintenance_cleanup(
...
begin
  ...
  cleanup_details := cleanup_details || jsonb_build_object('expired_closed_issues_deleted', deleted_count);

  delete from app_private.facility_reports
  where status in ('completed', 'unable-to-handle')
    and closed_at < now() - make_interval(days => closed_facility_days);
  get diagnostics deleted_count = row_count;
  cleanup_details := cleanup_details || jsonb_build_object('expired_closed_facilities_deleted', deleted_count);
...
create function app_api.run_maintenance_cleanup(
  valid_issue_categories text[] default null,
  retention_config jsonb default '{}'::jsonb
)
returns jsonb
...
create index if not exists issues_closed_retention_idx
  on app_private.issues (closed_at)
  where status in ('auto-rejected', 'review-rejected', 'infeasible', 'completed');
create index if not exists facility_reports_closed_retention_idx
  on app_private.facility_reports (closed_at)
  where status in ('completed', 'unable-to-handle');
create index if not exists role_assignment_audit_created_idx
  on app_private.role_assignment_audit (created_at);
